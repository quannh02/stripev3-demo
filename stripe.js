...
district.stripeCc = function ($) {
    ...
    self.createToken = function () {
        ...
        //Get the token
        stripeApi.createToken('bank_account', {
            country: 'US',
            currency: 'usd',
        ...
        }).then(function(result) {//Handle response
            if (result.error) {
                $('#stripe-cards-select-new')
                    .find('.district-error')
                    .remove()
                    .end()
                    .prepend('<p class="district-error">'+result.error.message+'</p>');
            } else {
                ...
                //Add token to form
                $inputs.cardToken.val(result.token.id);
