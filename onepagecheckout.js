...
OPC.prototype = {
    ...
    saveWhiteLabel: function () {
        var heidelpay_use_ccard = $j('#heidelpay_use_ccard').val();
        if('hpcc' == current_payment_method && heidelpay_use_ccard == '0' ){
            ...
            $j('#creditcard_iframe').contents().find('#submit').click();
    ...
    update: function (params) { // need to use timeout for chrome autocomplete
        /*var account_brand  = $j('#creditcard_iframe').contents().find('#account_brand').val();
        var account_number  = $j('#creditcard_iframe').contents().find('#account_number').val();
        var expiry_month    = $j('#creditcard_iframe').contents().find('#account_expiry_month').val();
        var expiry_year     = $j('#creditcard_iframe').contents().find('#account_expiry_year').val();
        var verification    = $j('#creditcard_iframe').contents().find('#account_verification').val();*/
        ...
        /*setTimeout(function(){
           *//* console.log(account_brand);
            console.log(account_number);
            console.log(expiry_month);
            console.log(expiry_year);
            console.log(verification);*//*
            if(account_brand == '') account_brand = 'MASTER';
            if(expiry_month == '') expiry_month = '-';
            if(expiry_year == '') expiry_year = '-';

            $j('#creditcard_iframe').contents().find('#account_brand').val(account_brand);
            if(account_number != ''){
                $j('#creditcard_iframe').contents().find('#account_number').val(account_number);
            }
            $j('#creditcard_iframe').contents().find('#account_expiry_month').val(expiry_month);
            $j('#creditcard_iframe').contents().find('#account_expiry_year').val(expiry_year);
            if(verification != ''){
                $j('#creditcard_iframe').contents().find('#account_verification').val(verification);
            }
        },9000);*/
    },
...
//
        ...
        checkout.setLoadWaiting(true);
        var points_amount = jQuery('[name="payment[points_amount]"]');
        if(typeof points_amount != 'undefined'){
            points_amount.attr('disabled', 'disabled');
        }
    ...
    setResponse: function (response) {
        var points_amount = jQuery('[name="payment[points_amount]"]');
        if(typeof points_amount != 'undefined'){
            points_amount.removeAttr('disabled');
        }
...
// init special rule if payment method changed        
    ...
    switchMethod: function (method) {
        ...
        var heidelpay_use_ccard = $j('#heidelpay_use_ccard').val();
        if('hpcc' == current_payment_method && (heidelpay_use_ccard == '0' ||  heidelpay_use_ccard == undefined)){
            var isValid = true;
